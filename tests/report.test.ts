...
describe('renderReport', () => {
  it('包含各区块、计数与格式化细节', () => {
    ...
    expect(text).toContain('☁️ 多云 22 ~ 29°C(当前 23.4°C)| 降水概率 40%');
    expect(text).toContain('🚨 暴雨橙色预警: 局部大暴雨');
    expect(text).toContain('📅 今日日程 (2)');
    expect(text).toContain('• 全天 中秋假期(日历)');
    expect(text).toContain('• 08:00-09:40 高等数学 @教三-101 · 第 1 周(课表)');
    expect(text).toContain('🚀 GitHub 新 Release (1)');
    ...
    expect(html).toContain('<b>🗞 订阅更新 (1)</b>');
  });
  ...
  it('百度热搜区块:名次、🆕 标记与 html 链接', () => {
    ...
    expect(html).toContain('🆕 热搜&lt;b&gt;词条B'); // 动态文本已转义
  });
